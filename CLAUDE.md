...
## Repo layout
- `index.html` — the entire app (~12,100 lines at v326: HTML + one big inline `<script>` + CSS). This is where almost all work happens.
...
2. **Edit incrementally** — never rewrite the whole file. Make targeted edits. **Never read `index.html` in full** (~715KB ≈ most of a context window) — grep for the target function/text and read only the relevant line ranges. For broad "where is X / does X exist / who calls Y" sweeps, delegate to the **`hearth-scout` subagent** (Sonnet, read-only) and work from its line anchors — but always read the exact lines you're about to edit yourself; the scout finds code, it doesn't vouch for understanding it.
...
5. **Commit, push, then immediately open a GitHub PR** — always, without waiting to be asked. The PR is how Cathal reviews the diff and merges to `main`. **Fill in `.github/pull_request_template.md` section for section** — that file is the canonical PR shape and the API does *not* apply it automatically, so reproduce it by hand. Two sections are mandatory on every PR:
   - **The "👉 Try this version" test link** (since v324), first line of the body: `https://raw.githack.com/coshea321/miniature-journey/<branch>/index.html` — it serves the branch with the in-app test mode active (orange banner, Firebase sync/login hard-blocked, no service worker, per-site sandboxed storage) **and, since v407, a fixed demo household already loaded**, reseeded fresh on every version, so Cathal never reviews against real data that wandered in from an earlier PR.
   - **A "Before you merge" checklist** (v407) = the template's five-box **"Always check"** block copied *verbatim, unchanged, every time* + a **"Check for this change"** block you write for this version: 2–4 tick boxes naming the section to open, the thing to tap, and what should happen, in language a non-coder can follow. If the change is invisible in the UI (a sync-timing fix, a merge guard), say so in one line and point at the tests instead of inventing a tap that proves nothing.
