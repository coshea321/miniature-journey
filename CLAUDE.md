...
- `.claude/` — the workflow tooling: `agents/hearth-preflight.md` + `agents/hearth-verifier.md` (subagents below), `commands/release.md` (`/release`), `skills/hearth-council/` (council review).
...
2. **Edit incrementally** — never rewrite the whole file. Make targeted edits. **Never read `index.html` in full** (660KB ≈ most of a context window) — grep for the target function/text and read only the relevant line ranges.
